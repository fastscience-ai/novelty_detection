...
def main(state_file):
    ...
    if state_file is None: #train
        if True:
            ...
            action = []
            reward = []
            folder_name =[ 'r_1', 'r_2', 'r_3', 'r_4', 'r_5'] #['r_4_2','r_1_1','r_1_3','r_2_1']#,'r_1_2','r_1_4','r_5_1']
            for name in folder_name:
            #if True:
                state_input, action_input, reward_output = collect_data_from_folder("./data/120000pts/100_level_0_type_2_novelties/"+name)
                reward_output = np.reshape(reward_output, [-1,1])
                for i in range(len(reward_output)):
                    if float(reward_output[i,0]) > 0.0:
                        #collecting non zero reward
                        state.append(state_input[i:i+1])
                        action.append(action_input[i:i+1])
                        reward.append(reward_output[i:i+1])
                        print(reward_output[i:i+1])
            ...
            print(np.shape(state), np.shape(action), np.shape(reward))
            np.save("state_nonzero4.npy", state)
            np.save("action_nonzero4.npy", action)
            np.save("reward_nonzero4.npy", reward)
